...
def _split_columns(df: pd.DataFrame, cfg: Dict[str, Any]) -> Tuple[List[str], List[str]]:
    ...
    num = [c for c in usable if is_numeric_dtype(df[c])]
    cat = [c for c in usable if c not in num]
    return num, cat
...
def _get_model_and_space(name: str, seed: int):
    ...
    if name == "ann":
        base = MLPRegressor(random_state=seed, max_iter=2000, early_stopping=True, n_iter_no_change=30)
        return (base, {"model__hidden_layer_sizes":[(64,32),(128,64),(256,128)],
